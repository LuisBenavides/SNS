...
```{r,echo=T,eval=F}
s = SNS(X=X,X.id=X.id, chart="EWMA", chart.par=c(0.01, 2.0171))
```
...
X = SNS::example84$X #get the dataset into a data frame
X.id = SNS::example84$X.id
s = SNS::SNS(X=X,X.id=X.id, chart="EWMA", chart.par=c(0.01, 2.0171))
plot(s)
```
...
Get the sns of the dataset using a EWMA scheme
```{r,echo=T,eval=F}
s = SNS(X=X,X.id=X.id, Y=Y, chart="EWMA", chart.par=c(0.01, 2.0171))
```
...
Y = SNS::example87$Y
s = SNS::SNS(X=X,X.id=X.id, Y=Y, chart="EWMA", chart.par=c(0.01, 2.0171))
plot(s)
```
...
Table with the dataset using dataframe of \code{R} (first 10 rows).
```{r, echo=FALSE, results='asis'}
knitr::kable(head(SNS::example49, 10))
```
...
```{r,echo=T,eval=F}
s = SNS(X=X,X.id=X.id, Y=Y, chart="Shewhart", scoring="Z-SQ",isFixed = TRUE)
```
...
```{r,echo=F,eval=T, results='hide', message=F, warning=F,fig.width = 9, fig.height = 6}
X = SNS::example49$X2
X.id = SNS::example49$X.id
Y = SNS::example49$Y2
s = SNS::SNS(X=X,X.id=X.id, Y=Y, chart="Shewhart", scoring="Z-SQ",isFixed = TRUE)
plot(s)
```
...
msns = SNS::MSNS(X, X.id, null.dist = "F")
plot(msns)
```
